...
Recorded macros are brittle, while general computer-use agents may synthesize actions beyond a user’s intended authority. We present Neloa, an open-source macOS system for teaching a workflow once and changing bounded details on later runs. Neloa aligns screen evidence, application/input events, optical character recognition, and timestamped narration. Local quantized Qwen3-VL annotates captured actions, proposes grounded missing inputs for review, and maps later instructions to declared variables. Teaching-time proposals remain evidence-grounded drafts; at run time, model output may replace only values on known variable steps, while action identifiers and approval gates remain fixed. We evaluate 4-bit and 8-bit tiers in a frozen 15-case synthetic benchmark with 61 executable assertions, 59 critical, across three trials per tier. <!-- ABSTRACT-RESULTS:BEGIN -->Across three fresh-process trials per tier, the 4-bit model passed 3/3 runs with a mean weighted score of 100.0%; the 8-bit model passed 3/3 with 100.0%. The two tiers had 0 critical assertion failures; peak process memory was 3.41 and 5.28 GiB, respectively.<!-- ABSTRACT-RESULTS:END --> The findings support 4-bit as the default within this benchmark, not general desktop reliability or end-user usability. Bounded adaptation offers a middle ground between exact replay and open-ended computer use while keeping recordings on device.

**Keywords:** programming by demonstration; desktop automation; human-AI interaction; intelligent agents; local AI; privacy
...
An anonymized artifact containing the source code, frozen evaluation manifest, synthetic fixture generator, machine-readable reports, and analysis script accompanies review. The evaluation does not contain personal recordings or third-party account data. The public repository URL will be restored after anonymous review. Neloa’s source is licensed under Apache License 2.0; pinned Qwen model artifacts retain their own upstream licenses.
