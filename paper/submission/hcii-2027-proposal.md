...
We ran three fresh-process trials for both the 4-bit and 8-bit model revisions with identical prompts (temperature 0.15; top-p 0.9). Reports retain failed trials rather than retrying them away. Collection used a 14-core Apple M4 Pro Mac with 48 GB memory and macOS 26.6.2. We record aggregate and model-only case pass rates, every critical failure, cached model-setup time, total evaluation time, peak resident memory, and pinned model size. <!-- PROPOSAL-RESULTS:BEGIN -->All six trials passed: both tiers achieved 100.0% model-case pass rates with 0 critical failures. Peak RSS was 3.41 GiB (4-bit) and 5.28 GiB (8-bit).<!-- PROPOSAL-RESULTS:END -->
