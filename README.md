...
`make model-eval-compare BASELINE=/path/to/baseline.json CANDIDATE=/path/to/candidate.json` produces a case-by-case JSON and Markdown comparison and exits nonzero for a real regression or an incompatible test contract. Different commits, model revisions, and quantization tiers can be compared as long as both reports contain the same cases and assertion definitions. See the [model evaluation guide](docs/MODEL_EVALUATION.md).
